...
*   **Python 3:** Studio dei fondamenti del linguaggio (sintassi base, data structures come liste e dizionari, control flow con if/while). Applicazione pratica dei comandi principali tramite lo sviluppo di script custom (es. `lab_monitor.py` tramite libreria `subprocess`) per il monitoraggio real-time dello stato degli asset di rete. [Python SOC Monitor](../Img/python-soc-monitor.png)
